...
export function calculateSolutionVolume(
  ...
  if (type === 'nutrient_amount') {
    ...
    if (targetEC < 1.2) {
      gramsPer100L = 5; intensity = 'ligera';
    } else if (targetEC >= 1.2 && targetEC < 2.0) {
      gramsPer100L = 10; intensity = 'moderada';
    } else {
      gramsPer100L = 20; intensity = 'alta';
    }
  ...
  if (type === 'dilution') {
    return (
      'Dilución para bajar EC — fórmula y ejemplo:\n' +
  ...
  if (type === 'water_change') {
    return (
      `Recomendación de cambios de agua (heurística):\n` +
