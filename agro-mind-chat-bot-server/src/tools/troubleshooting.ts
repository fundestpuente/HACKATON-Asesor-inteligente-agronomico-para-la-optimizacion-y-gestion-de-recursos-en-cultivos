...
export function troubleshootIssue(symptoms: string, cropType?:  string): string {
  if (!symptoms || symptoms.trim().length === 0) {
    return 'Por favor describe los síntomas observados para poder diagnosticar.';
  }

  const query = symptoms.toLowerCase();
  const results: Array<{ key: string; issue: any; score: number }> = [];

  for (const key of Object.keys(TROUBLESHOOTING_DATABASE)) {
    const issue = (TROUBLESHOOTING_DATABASE as any)[key];
    let score = 0;

    // Puntuar coincidencias simples entre palabras clave
    for (const s of issue.symptoms) {
      const sLower = s.toLowerCase();
      if (query.includes(sLower) || sLower.includes(query)) score += 2;
      // palabras individuales
      for (const word of query.split(/\W+/)) {
        if (sLower.includes(word) && word.length > 2) score += 1;
      }
    }

    if (score > 0) results.push({ key, issue, score });
  }
  ...
  return [`Diagnóstico posible: ${top.key.replace(/_/g, ' ')} (score: ${top.score})`,
    `Síntomas registrados:\n${issue.symptoms.map((s: string) => `• ${s}`).join('\n')}`,
    `Causas posibles:\n${issue.possible_causes.map((c: string) => `• ${c}`).join('\n')}`,
    `Soluciones sugeridas:\n${issue.solutions.map((s: string) => `• ${s}`).join('\n')}`,
    `Prevención:\n${issue.prevention.map((p: string) => `• ${p}`).join('\n')}`
  ].join('\n\n');
}
